...
for q in range(0,len(data2_per_sm[:,1])-2):
    for p in range(q+1,len(data2_per_sm[:,1])-1):
        ...
        Q_new1=abs(data2_per_sm[:,p]**2)
        Q_new2=abs(data2_per_sm[:,q]**2)
        Q_new12=abs((data2_per_sm[:,p]+data2_per_sm[:,q])**2)
        Q_new=(2**(2*2))*(Q_new1*Q_new2)/(Q_new12**2)
        Qxyw.append(Q_new)
Z=linkage(Qxyz,'ward')
plt.title('Hierarchical Clustering Dendrogram of Multivariate Time Series')
dendrogram(Z,labels=comparisons)
